...
/// the URL to UserDefaults under `flutter.stk_gate_tap_url`. The `flutter.`
...
class NativeTapBridge {
  static const String _key = 'stk_gate_tap_url';
  ...
  static Future<String?> consumeTapUrl() async {
    ...
    try {
      ...
      if (raw == null || raw.trim().isEmpty) {
        debugPrint('[STK.NATIVE] consumeTapUrl -> null');
        return null;
      }
      await prefs.remove(_key);
      debugPrint('[STK.NATIVE] consumeTapUrl -> "$raw"');
      return raw.trim();
    } catch (err) {
      debugPrint('[STK.NATIVE] consumeTapUrl failed: $err');
      return null;
    }
  }
}
